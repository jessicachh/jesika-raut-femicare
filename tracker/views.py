...
def doctor_appointment(request):
    ...
    pending_payment_verifications = list(
        Payment.objects.filter(
            ...
            status='pending',
        ).select_related('user', 'appointment', 'appointment__availability')
    )
...
def respond_appointment(request, appointment_id):
    ...
    if request.method == 'POST':
        ...
        # Cache the doctor profile once to avoid repeated DB hits below.
...
def payment_page(request, appointment_id):
    ...
    appointment = get_object_or_404(
        Appointment.objects.select_related('doctor', 'doctor__doctor_profile', 'doctor__payment_details', 'availability'),
        id=appointment_id,
        user=request.user,
    )
...
def submit_payment(request, appointment_id):
    ...
    appointment = get_object_or_404(
        Appointment.objects.select_related('doctor', 'doctor__doctor_profile'),
        id=appointment_id,
        user=request.user,
    )
    if appointment.status != 'awaiting_payment':
...
def get_notifications(request):
    # Fetch the 20 most recent notifications in a single query; derive the
    # unread count from the already-fetched list to avoid a second round-trip.
    notifications = list(
...
def doctor_dashboard(request):
    ...
    # Use a single queryset reference; aggregate counts in the DB to avoid
    # pulling all rows into Python just to call len().
    ...
    # Prefetch the related appointment (OneToOne reverse) so the template
    # can check slot.appointment without an extra query per row.
    availabilities = DoctorAvailability.objects.filter(
        doctor=request.user,
